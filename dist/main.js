"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var js_modules_1 = require("@wildebeest/js-modules");
var fs = require("fs");
var CommandService_1 = require("./CommandService");
var CliModule_1 = require("./CliModule");
var errors = 0;
function getModuleClasses() {
    return __awaiter(this, void 0, void 0, function () {
        var cliConfig, appModules, _a, _b, _i, key, _c, _d, e_1;
        return __generator(this, function (_e) {
            switch (_e.label) {
                case 0:
                    cliConfig = getConfig();
                    appModules = [CliModule_1.CliModule];
                    _a = [];
                    for (_b in cliConfig.modules)
                        _a.push(_b);
                    _i = 0;
                    _e.label = 1;
                case 1:
                    if (!(_i < _a.length)) return [3, 6];
                    key = _a[_i];
                    _e.label = 2;
                case 2:
                    _e.trys.push([2, 4, , 5]);
                    _d = (_c = appModules).push;
                    return [4, dynamicImport(key, cliConfig.modules[key])];
                case 3:
                    _d.apply(_c, [_e.sent()]);
                    return [3, 5];
                case 4:
                    e_1 = _e.sent();
                    errors++;
                    console.log("%s\x1b[33m%s\x1b[0m%s\x1b[33m%s\x1b[0m", "Cannot find module ", key, " in path ", cliConfig.modules[key]);
                    return [3, 5];
                case 5:
                    _i++;
                    return [3, 1];
                case 6: return [2, appModules];
            }
        });
    });
}
function getConfig() {
    var config = { modules: {} };
    var configFilePath = __dirname + '/../wildebeest.cli.json';
    if (fs.existsSync(configFilePath)) {
        var fileData = fs.readFileSync(configFilePath);
        config = JSON.parse(fileData);
    }
    else {
        throw new Error("Config file not found '" + configFilePath + "'.");
    }
    return config;
}
function dynamicImport(className, path) {
    return __awaiter(this, void 0, void 0, function () {
        var loadedClass;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4, Promise.resolve().then(function () { return require(path); }).then(function (classes) {
                        return classes[className];
                    })];
                case 1:
                    loadedClass = _a.sent();
                    return [2, loadedClass];
            }
        });
    });
}
function run(args) {
    return __awaiter(this, void 0, void 0, function () {
        var app, appModules, commandService;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    console.log("Loading CLI ...");
                    app = new js_modules_1.Application();
                    return [4, getModuleClasses()];
                case 1:
                    appModules = _a.sent();
                    app.run(appModules);
                    if (errors > 0) {
                        console.log("%s\x1b[32m%s\x1b[0m%s\x1b[33m%s\x1b[0m", "CLI ", "loaded", " with ", errors + " errors");
                    }
                    else {
                        console.log("%s\x1b[32m%s\x1b[0m", "CLI ", "loaded");
                    }
                    commandService = app.getContainer().get(CommandService_1.CommandService);
                    commandService.exec(args[2], args.slice(3));
                    return [2];
            }
        });
    });
}
exports.run = run;
//# sourceMappingURL=main.js.map